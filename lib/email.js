...
export async function enviarEmailConfirmacionSolicitud({ nombre, email }) {
  await resend.emails.send({
    from: FROM,
    to: email,
    subject: "Recibimos tu solicitud — Sabores del Monte",
    html: `
      <div style="font-family:sans-serif;max-width:520px;margin:0 auto;padding:24px;background:#0D0A06;color:#E2D0A8;border-radius:6px;">
        <h2 style="color:#C8793A;margin:0 0 12px;font-size:18px;font-weight:600;">¡Hola, ${nombre}!</h2>
        <p style="font-size:14px;line-height:1.6;margin:0 0 16px;">
          Recibimos tu solicitud para unirte al <strong style="color:#C8793A;">Club Origen</strong> de Sabores del Monte.
        </p>
        <p style="font-size:14px;line-height:1.6;margin:0 0 16px;">
          Nuestro equipo va a revisar tu información y te contactará en menos de <strong>48 horas</strong> para contarte los próximos pasos.
        </p>
        <p style="font-size:12px;color:#4B5563;margin:24px 0 0;">
          Sabores del Monte · Distribuidora artesanal
        </p>
      </div>
    `,
  });
